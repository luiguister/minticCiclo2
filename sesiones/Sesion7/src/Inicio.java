...
public class Inicio {
	...
	public static void main(String[] args) {
		...
		System.out.println("Digite codigo del producto a vender: ");
		codigoVenta = sc.nextLine();
		System.out.println("Digite unidades del producto a vender: ");
		unidades = sc.nextInt();
		sc.nextLine();
		for (int i = 0; i < vector.length; i++) {
			if (vector[i].getCodigo().equals(codigoVenta)) {
				if (vector[i].getCantidadB() < unidades) {
					System.out.println("No se puede realizar la venta");
				}
				else {
					System.out.println("Venta aprobada");
					vector[i].setCantidadB(vector[i].getCantidadB() - unidades);
					System.out.println("La factura con descuento es: "+vector[i].calcularTotalPagarVenta(unidades, true));
					System.out.println("La factura sin descuento es: "+vector[i].calcularTotalPagarVenta(unidades, false));
				}
			}
		}
	}
